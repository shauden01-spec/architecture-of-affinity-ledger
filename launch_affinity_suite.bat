...
if exist "datasets\affinity_core.db" (
    ...
    echo    ⚠️ Notice: Database core index not found.
)
echo.

echo [1/4] Booting Front-And-Center Core Desktop GUI Panel Interface...
start "Affinity Control Panel Dashboard" python affinity_gui.py

echo [2/4] Spinning up background file Sweeper Daemon service...
start "Affinity Sweeper Daemon" /min cmd /k "python sweeper_daemon.py"

echo [3/4] Launching real-time 0.1 Hz Pump Controller loop daemon...
start "Affinity Pump Automation Controller" cmd /k "python pump_automation.py"

echo [4/4] Booting Real-Time Telemetry Tracking Dashboard interface...
